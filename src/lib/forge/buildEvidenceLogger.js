...
export function formatBuildEvidence(buildOutput) {
  ...
  return [
    ...
    `ToolSelection: ${stringifyInput(buildOutput.toolSelection ?? null)}`,
    `PipelineNullAt: ${stringifyInput(buildOutput.pipelineNullAt ?? [])}`,
    `UnderservedAccessWarning: ${buildOutput.undServedAccessWarning === true}`,
    '',
    '## Auto-populated',
