...
3. **Per-product Doppler projects** (`saige`, `reltwin`, `reachsms`, `pressai`, `mypreglife`). Whitelisted in `CredentialAdapter`, never exercised. CEO call: are these provisioned in Doppler? Or whitelisted-only pending product-specific build work?
