...
| Dependency manifest safety | Migration Mode skips generated lockfiles such as `package-lock.json` during platform dependency scanning and treats package manifests as human-review-only, preventing automated GitHub writes to dependency metadata while app-layer migration continues. | Needs production Migration Mode evidence showing lockfile/package manifest findings no longer hard-fail the run. |
| Auth boundary migration safety | Direct Migration Mode classifies auth boundary component files such as `AuthContext.jsx` as human-review-only forbidden paths even when they live outside an `/auth/` directory, preventing automated writes to authentication/session code while app-layer migration continues. | Needs production Migration Mode evidence showing auth-boundary files are skipped instead of hard-failing GitHub writes. |
