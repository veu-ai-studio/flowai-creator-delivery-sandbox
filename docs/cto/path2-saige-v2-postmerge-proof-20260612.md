...
After the proof, CTO ran a read-only status probe against the Step 10 preview URL. Secret values were not printed.

Probe result:

```json
{
  "secretSource": "flowai/prd VERCEL_BYPASS_SECRET_SAIGE",
  "secretPresent": true,
  "secretLength": 32,
  "url": "https://saige-v2-p7cwizuu8-veu-ai-studio.vercel.app",
  "status": 401
...
Interpretation: the code and probe both attempted a Vercel Deployment Protection bypass, but the preview remained protected. This points to a likely project/secret alignment issue: the secret available to FlowAI may not be the generated bypass secret for the protected `saige-v2` Vercel project/deployment.

Official Vercel documentation checked on 2026-06-12 says the bypass value must be the generated secret for the project, and that `x-vercel-set-bypass-cookie` is optional for follow-up browser requests.
