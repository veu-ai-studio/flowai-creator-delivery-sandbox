...
Priority 2 axis wiring was merged and deployed. A CTO production smoke found the controls and route switching present, but the sidebar rendered the axes with compressed labels:
...
- `tests/ui/flowAIUnifiedShell.test.js`
  - Adds source assertions for the canonical sidebar labels.
...
2. Does this patch preserve the existing axis values and request/orchestrator behavior?
